...
<html>
	...
	function test_initial_inserts() {
		...
		if($output_array===$checkArray) echo "test_initial_inserts PASSED <br />";
	...
	function test_killing_gangster() {
		...
		if($expected_jail!==$calculated_jail) echo"not same jail";
		if($expected_mafia!==$calculated_mafia) echo "not same mafia";
		if(($expected_jail===$calculated_jail) && ($expected_mafia === $calculated_mafia)) echo "test_killing_gangster PASSED <br />";
	...
	function test_kill_simple() {
		...
		if($expected_jail!==$calculated_jail) echo"not same jail";
		if($expected_mafia!==$calculated_mafia) echo "not same mafia";
		if(($expected_jail===$calculated_jail) && ($expected_mafia === $calculated_mafia)) echo "test_kill_simple PASSED <br />";
	}
	...
	function test_goto_jail_no_samelevel_boss() {
		...
		$expected_jail = array(
		array("2.1",2,"1",1,"3.2.1",3,"3.2.2",3,"3.2.3",3 )
		);
		
		if($expected_jail!==$calculated_jail) echo"not same jail";
		if($expected_mafia!==$calculated_mafia) echo "not same mafia";
		if(($expected_jail===$calculated_jail) && ($expected_mafia === $calculated_mafia)) echo "test_goto_jail_no_samelevel_boss PASSED <br />";
	...
	function test_goto_jail_with_same_level_boss() {
		...
		$expected_jail = array(
		array("2.1-1",2,"1",1,"3.2.1",3,"3.2.2",3,"3.2.3",3 )
		);
		
		if($expected_jail!==$calculated_jail) echo"not same jail";
		if($expected_mafia!==$calculated_mafia) echo "not same mafia";
		if(($expected_jail===$calculated_jail) && ($expected_mafia === $calculated_mafia)) echo "test_goto_jail_with_samelevel_boss PASSED <br />";
	}
	...
	function test_release_jail() {
		...
		if($expected_jail!==$calculated_jail) echo"not same jail";
		if($expected_mafia!==$calculated_mafia) echo "not same mafia";
		if(($expected_jail===$calculated_jail) && ($expected_mafia === $calculated_mafia)) echo "test_release_jail PASSED <br />";
	...
	function test_release_jail_head() {
		...
		if($expected_jail!==$calculated_jail) echo"not same jail";
		if($expected_mafia!==$calculated_mafia) echo "not same mafia";
		if(($expected_jail===$calculated_jail) && ($expected_mafia === $calculated_mafia)) echo "test_release_jail_head PASSED <br />";
	}
	...
	function test_complex_release_jail() {
		...
		$expected_jail = array(
		array("b",2,"a",1,"c",3 ),
		array("f",5,"e",4,"g",6,"h",6)
		);
		
		if($expected_jail!==$calculated_jail) echo"not same jail";
		if($expected_mafia!==$calculated_mafia) echo "not same mafia";
		if(($expected_jail===$calculated_jail) && ($expected_mafia === $calculated_mafia)) echo "test_complex_release_jail PASSED <br />";
	}
	...
	function test_complex_release_jail2() {
		...
		if($expected_jail!==$calculated_jail) echo"not same jail";
		if($expected_mafia!==$calculated_mafia) echo "not same mafia";
		if(($expected_jail===$calculated_jail) && ($expected_mafia === $calculated_mafia)) echo "test_complex_release_jail2 PASSED <br />";
	}
	...
	function test_complex_release_jail_subordinate_root(){
		...
		if($expected_jail!==$calculated_jail) echo"not same jail";
		if($expected_mafia!==$calculated_mafia) echo "not same mafia";
		if(($expected_jail===$calculated_jail) && ($expected_mafia === $calculated_mafia)) echo "test_complex_release_jail_subordinate_root PASSED <br />";
	}
	...
	function test_complex_release_jail_with_kill(){
		...
		if($expected_jail!==$calculated_jail) echo"not same jail";
		if($expected_mafia!==$calculated_mafia) echo "not same mafia";
		if(($expected_jail===$calculated_jail) && ($expected_mafia === $calculated_mafia)) echo "test_complex_release_jail_with_kill PASSED <br />";
