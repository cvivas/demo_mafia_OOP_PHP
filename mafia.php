...
class mafia{
	...
	//Order of traversal is root, then rightest node, then leftest (kind of reverse preorder)
		...
		while (count($stack) > 0 ) {
			...
			$node = array_pop($stack); //remove first element of array
			if($node->getName() == $name){
				...
				foreach ($node->getSubordinates() as $value) {
					array_push($stack,$value); //add elements to the end of the array
	...
	function oldest_remaining_boss($level,$gangster_name){
		 ...
		 */  
		if($gangster_name == $this->root->getName()) return null;
		...
		while (count($queue) > 0 ) {
			...
			$node = array_shift($queue); //remove first element of array
			...
			elseif ($current_level<0) {
				...
				foreach ($node->getSubordinates() as $value) {
						array_push($queue,$value); //add elements to the end of the array
				}
			}
	...
	function release_gangster($name){
		 ...
		 * 
		 * */
		   $gangster = $this->release_gangster_under_boss($name); //place the gangster under the boss if posible
		   if(!isset($gangster)) return null; // gangster not found in jail.
		   $names = array();
    		 foreach ($gangster->getSubordinates() as $value) {
	...
	function subordinates_of_released_gangster($names, $gangster){
		...
		while (count($stack) > 0 && count($names)>0) {
			...
			if(in_array($node->getName(), $names)){
				...
				foreach ($node->getSubordinates() as $value) {
					array_unshift($stack,$value); //add elements to the end of the array
 ...
 * */
	function dump_tree($type=""){
		...
		if(isset($this->root))	$this->root->dump_html();
		echo '</div>';
	...
	function convert_dfs_string($tree){
		...
		while (count($stack) > 0) {
			...
			$node = array_shift($stack); //remove first element of array
